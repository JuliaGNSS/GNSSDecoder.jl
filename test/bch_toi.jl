...
@testset "BCH(51,8) TOI codec" begin
    ...
    @testset "Frozen golden codewords" begin
        ...
        # external anchor frozen from real broadcast symbols: the TOI 1/2/3
        # entries are the verbatim 52-symbol subframe-1 segments of a Spirent
        # GSS7000 post-FEC L1C recording (`nav_data_fec.L1_cnv`, GSS-CNAVDATA
        # format), the rest were generated by the corrected LFSR and
        # spot-checked against the recording's structure. They are packed
        # first-symbol-at-bit-0, matching `BCH_TOI_CODEWORDS`.
        #
        ...
        # ambiguity), and the modulo-400 wrap endpoint (399). Note t=0 is the
        # all-zeros codeword (the LFSR is seeded with 0) and t=256 its
