# Shared GPS CNAV core — IS-GPS-200N §30 ≡ IS-GPS-705J §20.3.3.
#
# The CNAV navigation message is defined once and broadcast identically on two
# signals: GPS L2C (on the L2 CM code, IS-GPS-200N §30) and GPS L5I (on the L5
# in-phase component, IS-GPS-705J §20.3). Both carry the *same* 300-bit
# message — same 8-bit preamble `10001011`, same rate-1/2 constraint-length-7
# (K=7) non-systematic convolutional (NSC) FEC (G1 = 0o171, G2 = 0o133), same
# 24-bit CRC-24Q over the whole message, same per-message-type bit layouts
# (MT10/11, 12–15, 30–37, 40), same π and the same `TOW × 6` scaling. The only
# differences are in the *signal* layer (L2C: 25 bps → 50 sps, 12-second
# messages; L5I: 50 bps → 100 sps, 6-second messages) and in which health bit
# `is_sat_healthy` reports — neither of which affects this symbol-domain
# decoder (the message is always 600 symbols, the sync window 616). This file
# therefore holds everything shared; `gps/l5.jl` and `gps/l2c.jl` are thin
# signal layers (their own `*Constants` type, decoder-state constructor, and
# `is_sat_healthy`). gnss-sdr and pocketsdr likewise reuse one CNAV decoder
# for both signals.
#
# Field offsets/scales are cited to IS-GPS-705J figures/tables below; the
# identical definitions appear in IS-GPS-200N §30 (e.g. Fig 30-1 ≡ Fig 20-1).
#
...
WGS 84 constants and CNAV message structure parameters shared by the GPS CNAV
signals. The CNAV message is identical on GPS L5I and GPS L2C, so the field
values are identical too; the phantom signal tag `S` (`:GPSL5I` / `:GPSL2CM`)
exists only so [`is_sat_healthy`](@ref) can dispatch on the signal (L5I reports
the L5 health bit, L2C the L2 health bit). The per-signal aliases
[`GPSL5IConstants`](@ref) and [`GPSL2CMConstants`](@ref) fix `S`.
...
Base.@kwdef struct GPSCNAVConstants{S} <: AbstractGNSSConstants
    ...
    """
    syncro_sequence_length::Int = CNAV_MESSAGE_SYMBOLS
    """
    Trailing next-message preamble segment retained for sync (16 symbols)
    """
    preamble_length::Int = CNAV_PREAMBLE_SYMBOLS
    """
    Mathematical constant π (IS-GPS-705J Table 20-II / IS-GPS-200N §30.3.3)
    """
    PI::Float64 = GNSS_PI
    """
...
"""
const CNAV_MESSAGE_BITS = 300
const CNAV_MESSAGE_SYMBOLS = 2 * CNAV_MESSAGE_BITS
"""
Trailing window: the next message's first 8 bits (16 symbols) confirm sync.
"""
const CNAV_PREAMBLE_BITS = 8
const CNAV_PREAMBLE_SYMBOLS = 2 * CNAV_PREAMBLE_BITS
const CNAV_WINDOW_SYMBOLS = CNAV_MESSAGE_SYMBOLS + CNAV_PREAMBLE_SYMBOLS  # 616
const CNAV_WINDOW_BITS = CNAV_MESSAGE_BITS + CNAV_PREAMBLE_BITS  # 308
...
packet is complete in itself, so `GPSCNAVData.reduced_almanacs` entries are
...
single message, so `GPSCNAVData.midi_almanacs` entries are inserted whole,
...
"""
Base.@kwdef struct GPSCNAVClockDifferentialCorrection
    PRN_a::Int
    t_op_D::Int
    t_OD::Int
    dc_data_type::Bool
...
"""
Base.@kwdef struct GPSCNAVEphemerisDifferentialCorrection
    PRN_a::Int
    t_op_D::Int
    t_OD::Int
    dc_data_type::Bool
...
"""
Base.@kwdef struct GPSCNAVIntegritySupportMessage
    GNSS_ID::Int
    WN_ISM::Int
    TOW_ISM::Int
    t_correl::Int
...
Decoded GPS CNAV navigation message data, shared by GPS L5I and GPS L2C (the
CNAV message is identical on both signals).

Holds the parameters decoded from CNAV message types 10, 11, 12, 13, 14, 15,
30-37, and 40 (IS-GPS-705J §20.3.3 ≡ IS-GPS-200N §30.3.3). The decoder fills
fields incrementally as the corresponding message types are received.
Field-naming follows [`GPSL1C_DData`](@ref) (CNAV-2 broadcasts nearly the same
parameter set): semi-circle quantities are converted to radians on decode
(multiplied by π), all `Union{Nothing,…}` until first decoded.
...
# GGTO (message type 35, Table 20-XI)
  ...
  - `GNSS_ID::Int64`: GNSS Type ID — 0 no data available, 1 Galileo, 2 GLONASS,
    3-7 reserved. The ICD directs that a reserved code be read as "the GGTO data
    to which it applies is presently unusable", so there is no BeiDou code to
...
"""
Base.@kwdef struct GPSCNAVData <: AbstractGPSCNAVData
...
Per-decoder cache shared by the GPS CNAV signals (GPS L5I and L2C).
...
$(TYPEDFIELDS)
"""
struct GPSCNAVCache <: AbstractGNSSCache
    """
...
GPSCNAVCache() = GPSCNAVCache(
    CircularDeque{Float32}(CNAV_WINDOW_SYMBOLS),
    Vector{Float32}(undef, CNAV_WINDOW_SYMBOLS),
...
function is_ephemeris_decoded(data::GPSCNAVData)
    # Message type 10
    !isnothing(data.WN) &&
        !isnothing(data.ΔA) &&
...
# Arguments
  ...
  - `state::GNSSDecoderState{<:GPSCNAVData}`: Current GPS CNAV decoder state
    (GPS L5I or L2C — both share the CNAV `GPSCNAVData`)
...
function reset_decoder_state(state::GNSSDecoderState{<:GPSCNAVData})
    ...
    GNSSDecoderState(
        state;
        raw_data = GPSCNAVData(state.raw_data; TOW = nothing),
        data = GPSCNAVData(),
...
const CNAV_VITERBI_NUM_STATES = 64  # 2^(K-1) for K=7
...
function _cnav_encoder_output(state::UInt8, u::UInt8)
    s1 = (state >> 5) & 0x01
    s2 = (state >> 4) & 0x01
    s3 = (state >> 3) & 0x01
    s5 = (state >> 1) & 0x01
...
const _CNAV_VITERBI_G1 = [
    Bool(_cnav_encoder_output(UInt8(s), UInt8(u))[1]) for
    s = 0:(CNAV_VITERBI_NUM_STATES-1), u = 0:1
]
const _CNAV_VITERBI_G2 = [
    Bool(_cnav_encoder_output(UInt8(s), UInt8(u))[2]) for
    s = 0:(CNAV_VITERBI_NUM_STATES-1), u = 0:1
]
...
@inline _cnav_branch_penalty(llr::Float32, y::Bool) =
    hard_slice(llr) != y ? abs(llr) : 0.0f0

"""
    gps_cnav_viterbi(soft_window) -> Vector{Bool}
...
function gps_cnav_viterbi(
    ...
    metrics::Vector{Float32} = Vector{Float32}(undef, CNAV_VITERBI_NUM_STATES),
    next_metrics::Vector{Float32} = Vector{Float32}(undef, CNAV_VITERBI_NUM_STATES),
    decisions::Matrix{Bool} = Matrix{Bool}(
        undef,
        CNAV_VITERBI_NUM_STATES,
    ...
    @inbounds for t = 1:num_steps
        ...
        llr2 = soft_window[2t]
        for sp = 0:(CNAV_VITERBI_NUM_STATES-1)
            ...
            m0 =
                metrics[ps0+1] +
                _cnav_branch_penalty(llr1, _CNAV_VITERBI_G1[ps0+1, u]) +
                _cnav_branch_penalty(llr2, _CNAV_VITERBI_G2[ps0+1, u])
            m1 =
                metrics[ps1+1] +
                _cnav_branch_penalty(llr1, _CNAV_VITERBI_G1[ps1+1, u]) +
                _cnav_branch_penalty(llr2, _CNAV_VITERBI_G2[ps1+1, u])
            from_odd = m1 < m0
        ...
        for i = 1:CNAV_VITERBI_NUM_STATES
...
"""
Sync result for a GPS CNAV message: the polarity-resolved 300-bit message and the detected polarity.
"""
struct GPSCNAVSync
    """
...
function _pack_preamble(bits::Vector{Bool}, start::Int)
    candidate = 0x00
    @inbounds for i = 0:(CNAV_PREAMBLE_BITS-1)
...
"""
    try_sync(state::GNSSDecoderState{<:GPSCNAVData}) -> Union{Nothing,GPSCNAVSync}

Per-signal sync hook for the GPS CNAV signals (L5I / L2C). Viterbi-decodes the buffered 616-symbol
...
message. Returns the [`GPSCNAVSync`](@ref) on a match (carrying the message
...
function try_sync(state::GNSSDecoderState{<:GPSCNAVData})
    ...
    if leading == CNAV_PREAMBLE && trailing == CNAV_PREAMBLE
        polarity_flipped = false
    elseif leading == ~CNAV_PREAMBLE && trailing == ~CNAV_PREAMBLE
        ...
        @inbounds for i = 1:CNAV_MESSAGE_BITS
            bits[i] = !bits[i]
        end
    else
        return nothing
    end
    crc24q(view(bits, 1:CNAV_MESSAGE_BITS)) == 0 || return nothing
    word = UInt320(0)
    @inbounds for i = 1:CNAV_MESSAGE_BITS
        word = (word << 1) | UInt320(bits[i])
    end
    return GPSCNAVSync(word, polarity_flipped)
end

"""
    complement_buffer_if_necessary(state::GNSSDecoderState{<:GPSCNAVData}, sync)

Resolve the 180° polarity ambiguity for a GPS CNAV message. `sync::GPSCNAVSync` already
...
"""
    decode_syncro_sequence(state::GNSSDecoderState{<:GPSCNAVData}, sync::GPSCNAVSync)
...
fields.
"""
function decode_syncro_sequence(state::GNSSDecoderState{<:GPSCNAVData}, sync::GPSCNAVSync)
    word = sync.word
    word_length = CNAV_MESSAGE_BITS
    PI = state.constants.PI

    message_id = Int(get_bits(word, word_length, 15, 6))
    # The 17-bit message TOW count, in units of 6 seconds, is the SV time at the
    # start of the *next* message — the next message is 6 s away on L5I and 12 s
    # on L2C, but the count's LSB is 6 s on both (IS-GPS-705J §20.3.3 /
...
"""
function parse_mt10(raw::GPSCNAVData, word::UInt320, PI::Float64)
    word_length = CNAV_MESSAGE_BITS
    GPSCNAVData(
        raw;
...
"""
function parse_mt11(raw::GPSCNAVData, word::UInt320, PI::Float64)
    word_length = CNAV_MESSAGE_BITS
    GPSCNAVData(
        raw;
...
"""
function parse_clock_block(raw::GPSCNAVData, word::UInt320)
    word_length = CNAV_MESSAGE_BITS
    GPSCNAVData(
        raw;
...
"""
function parse_mt30(raw::GPSCNAVData, word::UInt320)
    word_length = CNAV_MESSAGE_BITS
...
"""
function _cnav_reduced_almanac_packet(
    word::UInt320,
    start::Int,
    WN_a::Int,
    t_0a::Int,
...
"""
function parse_mt32(raw::GPSCNAVData, word::UInt320)
    word_length = CNAV_MESSAGE_BITS
    raw = parse_clock_block(raw, word)
    GPSCNAVData(
        raw;
...
"""
function parse_mt33(raw::GPSCNAVData, word::UInt320)
    word_length = CNAV_MESSAGE_BITS
    raw = parse_clock_block(raw, word)
    GPSCNAVData(
        raw;
...
"""
function parse_mt35(raw::GPSCNAVData, word::UInt320)
    word_length = CNAV_MESSAGE_BITS
    raw = parse_clock_block(raw, word)
    GPSCNAVData(
        raw;
...
function _cnav_cdc_packet(
    ...
    PRN_a == CNAV_DC_EMPTY_PRN && return nothing
    GPSCNAVClockDifferentialCorrection(;
        PRN_a,
        t_op_D,
        t_OD,
...
"""
function parse_mt15(raw::GPSCNAVData, word::UInt320)
    word_length = CNAV_MESSAGE_BITS
    GPSCNAVData(
        raw;
...
"""
function parse_mt36(raw::GPSCNAVData, word::UInt320)
    word_length = CNAV_MESSAGE_BITS
    raw = parse_clock_block(raw, word)
    GPSCNAVData(
        raw;
...
"""
function parse_mt40(raw::GPSCNAVData, word::UInt320)
    word_length = CNAV_MESSAGE_BITS
    ...
    ism = GPSCNAVIntegritySupportMessage(;
...
"""
    validate_data(state::GNSSDecoderState{<:GPSCNAVData})
